        ...
        if changed_query.contains(*entity) {
            ...
            match locus.position {
                Position::Point(pos) => {
                    ...
                    let anim =
                        LerpVec3::from_translation(transform.translation, target.translation, 6);
...
pub fn mob_movement(
    mut commands: Commands,
    mut sprite_query: Query<(Entity, Option<&PlayerAvatar>, &mut LerpVec3, &mut Transform)>,
    ...
    for (sprite_entity, _player, mut anim, mut transform) in sprite_query.iter_mut() {
        if anim.current_frame == 1 {
            ...
            commands.entity(sprite_entity).remove::<LerpVec3>();
        } else {
            transform.translation.x += anim.delta.x;
            transform.translation.y += anim.delta.y;
            let k = anim.total_frames as f32 / 2.0;
            let scale_factor = (k - (anim.current_frame as f32 - k).abs()).abs() / 35.0 + 1.0;
